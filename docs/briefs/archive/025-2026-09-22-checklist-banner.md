...
Head `c8e148a98579088b0d72114d1250ab0fd88718ba`, base
`58be9808131d0cdc94a6f7f5ac795e9d6f34193b`.

**Result.** The README banner is a checklist: one row per started format,
written column headers, one drawn glyph per cell (no font-dependent
characters), a five-entry legend, one summary line. Tests re-derive each
cell's glyph from the brief's design table and fail if complete and verified
render alike; both were shown red by the Verifier with its own mutations. The
full atlas is byte-identical to base.

**Brain's defect.** The Verifier prompt carried a base SHA Brain had typed out
from a short hash; it did not exist. The Verifier stopped, checked with the
owner, and used the real base. Prompts now take SHAs only from `git rev-parse`
output.

**Rendered images** existed only in the Builder's own session scratch folder,
which is why the Verifier could not find them; Brain located and viewed them,
and they match the pushed banner.

**Carried forward (round 26):** the row dates are Format Library catalogue
dates (for example 05.08 for Goat), which are not this project's canonical
`period.start` (April 2005), and `YY.MM` reads poorly.
