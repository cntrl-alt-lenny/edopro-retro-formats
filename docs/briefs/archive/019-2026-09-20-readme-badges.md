...
Head `f879124cb8fbc0c54d9e966444b722b56e0311d8`, base
`42d8cee0e903a6e4f02ac0f2b8d138aa32153ea3`. The first delivery (`8d0ae1a`)
was returned for two defects neither role raised: badge alt text hard-coding
changeable values, and "progress 7/12 areas" readable as overall project
progress beside the 128-format catalogue banner.

Badge row now: CI (GitHub Actions) → canonical areas (shields endpoint over a
generated JSON) → python 3.10+ (static runtime floor) → license (GitHub).
The progress value is generated from `formats/*/format.json`, guarded by
`generate_progress_badge.py --check` and a test; the alt text is value-free
and test-pinned.

**Confirmed after merge by Brain**, rendering from `main`: "CI – passing",
"canonical areas: 7/12 complete", "python: 3.10+", "license: MIT".

**Owner's taste call, left open:** "canonical" is this project's own word; a
newcomer-friendlier label such as "3 formats: 7/12 areas complete" is a
one-line change for a later round if wanted.
