...
## Outcome — accepted 2026-09-21 on the second delivery, merged with owner approval

Head `66c9a0d3f32d6c0d7ce2ba5e126396708e25798b`, base
`b5a5c0065c8179e80b23998f9455976fda9796d0`. The first delivery (`1733d7c`)
was historically sound but left `validate` red: bounding the activation axis
made state `c1` determinate for five records, and this brief had forbidden the
coverage acknowledgement the validator requires. **That conflict was Brain's
brief defect**; Amendment 1 authorized the acknowledgement.

**Result — the first change to historical data since the framework adoption.**
Freed the Matchless General, Fusion Sage, Horus the Black Flame Dragon LV4,
Thunder Dragon and Toon Table of Contents are now determinate at the Edison
snapshot from their own 2008-12-16 Konami Card FAQ entries (Freed and Horus
also carry 2005-07-01 UDE old-state entries), and are acknowledged as known
divergences: the modern scripts enforce the new activation rule and cannot
reproduce the old Deck-verification procedure. **Edison's shipped output is
unchanged** — the modern implementations were selected before and still are —
so this is better accounting, not different gameplay. Brain's earlier
expectation that it would be "more faithful" was wrong and is corrected here.

**Remaining 33 records stay ambiguous**, each with its reason recorded: 15 have
no own entry in the UDE replay or the eight 2008 Konami FAQ pages; the rest
have entries that address timing, cost or resolution but not activation with
nothing to find. No card's date was inferred from another's.

**Brain re-derived:** Fusion Sage's own FAQ entry at its served memento, and —
settling the Verifier's one unproven claim — the five modern scripts at the
pinned CardScripts revision (`official/`), all gating activation on
`Duel.IsExistingMatchingCard` with no `GoatConfirm` path.

**Carried forward:** the migration test reads the five expected payloads from
commit `1733d7c`, so the suite needs full history; a committed fixture would
