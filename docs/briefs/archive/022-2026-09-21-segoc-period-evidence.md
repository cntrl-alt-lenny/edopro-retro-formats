...
Head `1244cdbbfbabf86556569ed7d3737f2adf93f416`, base
`118d75e00ddbd7e6864c85b245914d0291ed4d33`. The first delivery (`d458d09`) was
returned for two provenance defects the Verifier found and Brain confirmed:
row 9 had lost its v6.0–v8.0 byte-identical rulebook bracket, and the KDE
policy PDF was cited from a third-party copy with no source record.

**Result.** No official or officially sanctioned 2009–2010 material found —
rulebooks, event FAQs, rulings documents, KDE's February 2010 tournament
policy — addresses hidden-zone trigger folding or the
earliest-simultaneous-event restriction. Edison's rule profile is unchanged;
both flags stay unresolved, now with the search recorded per mechanic. Round
21's carried wording defect is fixed: all 12 HTTP-200 and 29 HTTP-302 rows are
stated with their destinations.

**Brain re-derived** evidence retention by comparing both research documents
line by line with the base; only the corrected sentence and the rewritten row
9 differ, and every claim of the old row is present in the new one.

**Pattern.** The evidence-retention defect recurred for the fourth time, the
first since `AGENTS.md` made it a standing rule: the Builder did not treat a
research-table row as a "record". The amendment made that explicit. Unregistered
citations have now needed a return twice (rounds 18 and 22); round 23 adds a
mechanical check for that class.
