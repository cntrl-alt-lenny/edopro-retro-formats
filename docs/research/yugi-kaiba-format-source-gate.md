...
- **Restriction list:** best current hypothesis is a tournament-specific
  ...
  **RE-DERIVED 2026-08-30 (session 5), CORRECTED 2026-08-30 (session 6),
  CORRECTED AGAIN 2026-08-30 (session 7):** session 5 re-examined the
  original three-axis model against what an August-26-Tokyo-Dome-*snapshot*
  banlist artifact requires and concluded `effective_date` only needs to
  satisfy `effective_date <= snapshot`, and that H2-vs-H3 (finals-only vs.
  ...
  **actually** in force" shows `effective_date` is documented to mean the
  ...
  independent of what bytes get generated. Session 6 restored both
  propositions to load-bearing and added a seventh axis,
  `schema_representability_status`, but session 6's own justification for
  that axis conflated H3 ("other tournament-specific scope") with H1
  ("general OCG list") - a real error external review found. **Session 7
  corrected this** and, separately, corrected a load-bearing conflation in
  `target_recommendation` itself: `snapshot` had held `1999-08-25` (the
  pool-cutoff-motivated day *before* the event), not the event date the
  field is documented and precedented (see Edison's own `format.json`) to
  hold - moved to `1999-08-26`, with the pool-cutoff date preserved
  separately as a new `pool_cutoff` field, still `1999-08-25`. Verified
  mechanically safe: every erratum in the corpus (not just the six
  pool-relevant ones) resolves identically at both dates. Separately,
  `outer_scope_status` was split into two genuinely distinct propositions -
  `scope_class_status` (S1: general-regional H1 vs. tournament-specific H2
  **or** H3 - the proposition that actually determines whether an ordinary
  region-scoped banlist object is honest) and `tournament_extent_status`
  (S2: given tournament-specific, finals-only H2 vs. whole-tournament-run
  H3 - a refinement of S1, not a substitute for it) - and
  `schema_representability_status` is now modelled explicitly as DERIVED
  from `scope_class_status` (not `tournament_extent_status`, and not an
  ...
  "World Tournament 8.26 in TOKYO DOME, Final Recruitment!!" and the TOC
  confirms the article sits at page 20, but a cover promoting an event
  proves the *issue* discusses that event, not that the *restriction was
  in force there* - that bridge remains supplied only by the interior crop
  and the two independent retrospectives, so these two sources were moved
  out of `target_event_applicability_status` and retained only under
  `source_authentication_status` (issue existence/credibility). Session 7
...
## Temporal/scope-model correction (session 7, 2026-08-30)

This pass's mandate was narrower than sessions 4-6: not a new source hunt,
but a correction of the temporal and scope MODEL underneath everything
those sessions had already found. Two conceptual bugs, both introduced by
this research chain's own prior sessions, were found and fixed.

### Phase A-B: the temporal model, decomposed

The single word "snapshot" had been implicitly standing in for several
genuinely distinct dates. Re-reading `schemas/format.schema.json`,
`schemas/pool.schema.json`, `schemas/banlist.schema.json`,
`docs/format-schema.md`, `retroformats/validate.py`, and `retroformats/
model.py` in full, eight distinct temporal concepts were named explicitly
(recorded in full at `restriction_list_current.temporal_model_2026_08_30_
session7.T1_through_T8_temporal_concepts`): the defining historical event
date (T1); `format.period.start` (T2); `format.period.snapshot` (T3);
`pool.cutoff.cutoff_date` (T4); the date against which erratum
applicability is evaluated (T5); `banlist.effective_date` (T6); a
historical card-legality cutoff if it differs from physical availability
(T7); and a community-retrospective reference date if it differs from the
historical-event model (T8).

Two explicit hypothetical `Format`/`Banlist`/`Pool` objects were built
in-memory (never written to `data/`/`formats/`) and run through the REAL
`retroformats.validate.Validator._validate_format()`:

- **Model A** (`snapshot=1999-08-25`, `pool.cutoff=1999-08-25`,
  `banlist.effective_date=1999-08-26`): rejected, with the validator's
  actual `format.banlist-not-in-force` error (banlist not yet in force at
  the format's own snapshot date).
...
`target_recommendation.snapshot` held `1999-08-25` - the day BEFORE the
event, chosen because it matched the desired pool cutoff, not because it
was the format's own defining-event date. This directly contradicted every
banlist re-derivation in sessions 5-6, which correctly reasoned about a
hypothetical `period.snapshot = "1999-08-26"`. `schemas/format.schema.json`
documents `period.snapshot` as "usually the date of the format's defining
event," and this repository's OWN Edison precedent (`formats/2010-03-
edison/format.json`) states outright that its snapshot "is SJC Edison day
1 (2010-04-24), the event the format is named for" - with its pool cutoff
independently 16 days later, for an unrelated reason. T3 and T4 are
already, precedentedly, independent numbers in this codebase; conflating
them for Tokyo Dome was the actual root cause of the Aug-25/Aug-26
inconsistency.

Before changing anything, every pool-relevant erratum was mechanically
re-checked at both candidate dates - not assumed safe. Using the REAL
`retroformats.repo.Repository.load()` and each erratum's real
`selection_at()`, ALL 296 errata in the live corpus (not merely the six
previously flagged as pool-relevant) were compared at `date(1999, 8, 25)`
and `date(1999, 8, 26)`: **0 of 296 produce a different result.** (The
"six pool-relevant errata" starting list itself was traced to
`superseded_findings.rejected_2026_08_rules_and_restriction_research.
errata_intersection_audit` - archived, not live data; the exhaustive
296-record check does not depend on trusting that archived list's own
membership claim.)

`target_recommendation.snapshot` was moved to `1999-08-26` (T3, the
defining event date). A new `target_recommendation.pool_cutoff` field
holds `1999-08-25` (T4/T8), unchanged, matching `card_pool.cutoff` exactly
- the 370-card certified pool and its digest
(`f65d30b07d231c1a1913b36b659dfc8e6d536fb2c7db0ffa36cd65f6e57ba1eb`) never
moved. Nothing in `data/`, `formats/`, or `dist/` was touched - this is a
research-packet metadata correction only.

### Phase D-E: the scope taxonomy, re-derived

Session 6 conflated H3 ("other tournament-specific scope," including
qualifying rounds - per `scope_hypotheses`' own definition) with H1
("general OCG list") when justifying `schema_representability_status` -
treating an H3 finding as if it would make an ordinary region-scoped
banlist object "comparatively defensible... because they genuinely ARE
general policies." H3 is a WIDER tournament population than H2, but it is
still a tournament population, not a regional one.

The old `outer_scope_status` axis is now split into two propositions that
were never actually the same thing:

- **`scope_class_status`** (S1) - was the restriction a general OCG
  regional policy (H1), or a tournament/event-specific policy (H2 **or**
  H3, since both are forms of tournament-specific policy per the packet's
  own hypothesis labels)? This is the proposition that determines whether
  an ordinary reusable region-scoped `banlist.schema.json` object is a
  truthful shape for the fact at all.
- **`tournament_extent_status`** (S2) - GIVEN tournament-specific, was it
  finals-only (H2) or the whole tournament programme (H3)? A refinement of
  S1, not a substitute for it - resolving H3 over H2 widens which PART of
  the tournament series was covered, it does not establish general-
  regional policy.

`schema_representability_status` is now modelled explicitly as DERIVED
from `scope_class_status`, not `tournament_extent_status`, via a
documented `state_machine`: general-regional (H1) proven would make the
existing schema sufficient (`RESOLVED_EXISTING_SCHEMA_SUFFICIENT`);
tournament-specific (either H2 or H3) proven leaves it structurally
insufficient regardless of extent; unresolved (the current state, with the
evidentiary lean AGAINST H1) leaves it BLOCKING/conditional. It is no
longer listed in `canonicalization_status.load_bearing_axes` - it is
tracked separately in a new `canonicalization_status.derived_axes` field
with an explicit `depends_on: "scope_class_status"`.

### Phase F: the corrected, dependency-aware gate model

`canonicalization_status.load_bearing_axes` now lists SEVEN
independently-evidenced propositions: `content_membership_status`,
`content_completeness_status`, `target_event_applicability_status`,
`source_authentication_status`, `scope_class_status`,
`tournament_extent_status`, `first_effective_date_status`.
`schema_representability_status` is the sole entry in the new
`derived_axes` field, computed from `scope_class_status` rather than
independently asserted - encoding "a dependency, not a flat AND"
structurally, not just in prose. `remaining_blockers` was corrected to
stop claiming all axes were uniformly `SUPPORTED_BUT_INCOMPLETE` (
`schema_representability_status` is actually `BLOCKING`) and to stop
implying an H2-vs-H3 resolution alone could close the schema gap (only
`scope_class_status` drives it). `canonicalization_status.status` remains
`UNRESOLVED_BLOCKING` throughout - the correction changes which exact
propositions are load-bearing and clarifies one dependency; it removes no
blocking reason.

### Phase G: marketplace-prose cleanup

Session 6 had already moved the Yahoo Auctions / Mercari `source_ids` out
of `target_event_applicability_status` (into `source_authentication_status`
only), but the descriptive PROSE in
`provenance_independence_assessment.independence_groups` and
`source_contemporaneity_ledger` had not caught up - both still said these
sources were "cited by `target_event_applicability_status`" or
"establish[ed]... target-event applicability... evidence," directly
contradicting the corrected `source_ids` lists. Checking `source_ids`
alone would have missed this entirely. Both fields were corrected to state
plainly that these sources establish issue-existence/context evidence
only (the issue is ABOUT the event) and explicitly do NOT establish
restriction-applicability evidence (that the restriction was IN FORCE at
the event) - matching session 6's own already-correct `source_ids`
placement. A new structural test,
`_assert_context_only_sources_not_described_as_applicability_evidence`,
guards against this specific class of drift going forward: it checks both
the structural record (ledger entries and independence groups for
`context/authentication-only`-tier sources) and a whole-packet textual
scan, not `source_ids` membership alone.

### Phase H: the future-date regression, closed for real

The Phase-0 guard (`_assert_no_active_metadata_dated_after_session_date`,
originally added session 5) only ever walked `_walk_strings` - string
VALUES. Directly reproduced before fixing: a mutation that plants a
future-dated dictionary KEY (`packet[...]["adversarial_review_2026_09"] =
{...}`) was NOT caught by the pre-fix helper. Fixed by adding a `_walk_keys`
walk that inspects dictionary keys at every level, with no hedge exemption
for keys (an identifier cannot carry a "this is deliberately hypothetical"
qualifier the way prose can) - only the `superseded_findings` archival
exemption still applies. Five mutation tests now cover: a future date in a
string value (rejected), in a dictionary key (rejected), a legitimate
historical date in either form (accepted), explicitly hypothetical future
prose (accepted), and a future date inside `superseded_findings` (exempt,
per the stated archival policy).

### Adversarial review (session 7)

Dispatched against nine specific attempted counter-claims, each required to
be checked against actual schema/code/packet state or in-memory model
objects rather than accepted on the packet's own narrative. Every
substantive finding was personally reproduced before being trusted. Full
exchange in `restriction_list_current.contemporaneous_source_investigation_
2026_08_30.adversarial_review_2026_08_30_session7`.

Eight of the nine attacks were REFUTED - the corrective pass's underlying
claims held up under direct, independently-reproduced inspection (the
Aug-25/Aug-26 rationale against the actual schema text and Edison
precedent; pool independence from the snapshot field; H3's own "other
tournament-specific scope" label directly contradicting the "H3 means
general policy" strawman; the scope_class_status/tournament_extent_status
split's non-redundancy, confirmed by `schema_representability_status`'s
state machine branching only on the former; the marketplace-prose cleanup,
confirmed exhaustive across 36 occurrences; the future-date-key fix,
confirmed by executing the real function against a real mutation before
and after the fix; and the "0 of 296 errata change" result, independently
recomputed from a fresh `Repository.load()` with a sanity control proving
the comparison can detect a real difference when one exists).

**One attack found a genuine, reproducible gap, since fixed:** checking
that `schema_representability_status.depends_on` structurally names
`scope_class_status` is not the same as checking that its cleared VALUE is
actually consistent with which hypothesis `scope_class_status` resolved
to. A constructed mutation - `scope_class_status.status = "PROVEN"` with
prose describing a tournament-specific finding, `schema_representability_
status` separately (and, per its own documented state machine, wrongly)
flipped to its cleared status - passed every existing check undetected,
personally reproduced before and confirmed fixed after. Fixed by adding
`scope_class_status.resolved_hypothesis` (`null` while unresolved; `"H1"`/
`"H2"`/`"H3"` once proven) as the single machine-checkable source of truth,
plus a value-consistency check requiring `resolved_hypothesis == "H1"`
before `schema_representability_status` may claim its cleared status, plus
a new regression test
(`test_mutation_AH5_schema_representability_cleared_while_scope_class_
proven_against_h1_is_rejected`).

One further, unrelated bug was found and fixed outside the nine attacks: a
citation in `target_recommendation.snapshot_note` pointed to a nonexistent
path (`schemas/format-schema.md`, a garbled merge of the real schema file
and the real prose doc) - corrected to cite `schemas/format.schema.json`'s
`period.snapshot` description specifically, where the quoted phrase
actually lives. The underlying substantive claim was never wrong, only
that one citation string.

None of the findings loosened `canonicalization_status`, which remains
