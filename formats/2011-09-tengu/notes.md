...
- **Errata — partial, and computed.** Applicability is computed from each record's
  ...
  - 126 determinate records: 33 modern, 52 historical substitutions (`reuse-upstream`),
    38 that no upstream card implements: 32 acknowledged divergences (`known-gap`) and,
    since round 031, 6 generated by this repository (`custom-script`, `data/custom-cards/`:
    Goddess of Whim, Strike Ninja, Green Baboon, Rise of the Snake Deity, Malefic
    Blue-Eyes White Dragon, Soul Rope), plus 3 `none-needed`.
  ...
  - Zero hand-authored erratum overrides; all 52 upstream substitutions and the 6
    generated ones arise mechanically from the canonical v2 errata corpus, so the list
    names 58 historical codes in place of modern ones.
